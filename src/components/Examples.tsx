...
const examples: { label: string; conversation: Msg[] }[] = [
  ...
  },
  {
    ...
    conversation: [
      ...
      { type: "img-real", src: "/stickers/quentli-logo.png", caption: "logo.png" },
      { type: "in", text: "¡Perfecto! Aquí está tu sticker de marca 🧡" },
      { type: "sticker-img", src: "/stickers/quentli-sticker.webp", alt: "Sticker logo" },
    ],
  },
];
  ...
  return (
    ...
    >
      {conversation.map((msg, i) => {
        if (msg.type === "in") {
          return (
            <div key={i} style={{ alignSelf: "flex-start", background: "#202c33", color: "#e9edef", padding: "5px 9px", borderRadius: "8px 8px 8px 2px", fontSize: 11.5, maxWidth: "78%" }}>
              {msg.text}
            </div>
          );
        }
        if (msg.type === "out") {
          return (
            <div key={i} style={{ alignSelf: "flex-end", background: "#005c4b", color: "#e9edef", padding: "5px 9px", borderRadius: "8px 8px 2px 8px", fontSize: 11.5, maxWidth: "78%" }}>
              {msg.text}
            </div>
          );
        }
        if (msg.type === "img") {
          return (
            <div key={i} style={{ alignSelf: "flex-end", background: "#2a3942", borderRadius: 8, width: 90, height: 70, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", fontSize: 22, gap: 2 }}>
              {msg.emoji}
              <span style={{ fontSize: 9, color: "#8696a0" }}>{msg.caption}</span>
            </div>
          );
        }
        if (msg.type === "img-real") {
          return (
            <div key={i} style={{ alignSelf: "flex-end", borderRadius: 10, overflow: "hidden", width: 90, height: 90, background: "#1f2c34", position: "relative", flexShrink: 0 }}>
              <Image src={msg.src} alt={msg.caption} fill style={{ objectFit: "cover" }} />
              <div style={{ position: "absolute", bottom: 4, right: 6, fontSize: 9, color: "rgba(255,255,255,0.7)" }}>{msg.caption}</div>
            </div>
          );
        }
        ...
        if (msg.type === "sticker") {
          return (
            <div key={i} style={{ alignSelf: "flex-start", fontSize: 44, padding: 4, filter: "drop-shadow(0 3px 8px rgba(0,0,0,0.5))" }}>
              {msg.emoji}
            </div>
          );
        }
        if (msg.type === "sticker-out") {
          return (
            <div key={i} style={{ alignSelf: "flex-end", fontSize: 44, padding: 4, filter: "drop-shadow(0 3px 8px rgba(0,0,0,0.5))" }}>
              {msg.emoji}
            </div>
          );
        }
        if (msg.type === "sticker-labeled") {
          return (
            <div key={i} style={{ alignSelf: "flex-start", position: "relative", fontSize: 44, padding: 4, filter: "drop-shadow(0 3px 8px rgba(0,0,0,0.5))" }}>
              {msg.emoji}
              <div style={{ position: "absolute", bottom: 2, left: 0, right: 0, textAlign: "center", fontSize: 9, fontWeight: 900, color: "white", textShadow: "0 1px 3px rgba(0,0,0,0.9)", letterSpacing: "0.06em" }}>
                {msg.text}
              </div>
            </div>
          );
...
export default function Examples() {
  return (
    <section
      id="ejemplos"
      style={{ paddingTop: 100, paddingBottom: 100, background: "#161616", position: "relative", overflow: "hidden" }}
    >
      <div className="spray-circle" style={{ width: 600, height: 600, background: "#FF8100", top: "50%", left: -200, transform: "translateY(-50%)" }} />

      <div className="max-w-7xl mx-auto px-4 sm:px-6">
        <div style={{ textAlign: "center", marginBottom: 60 }}>
          <div className="section-tag">Ejemplos</div>
          <h2 className="font-heading" style={{ fontSize: "clamp(36px, 5.5vw, 68px)", color: "#F5F5F5" }}>
